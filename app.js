...
fetch('https://api.sebin.ai/getdata').then(function(response) {
	return response.json();
}).then(function(json) {
	data = json
	var covid_us_timeline = data.Data[1]
    var covid_vaccine = data.Data[0]
	am4core.ready(function() {
		...
		var people_vaccinatedColor = am4core.color("#d21a1a");
		var people_fully_vaccinatedColor = am4core.color("#1c5fe5");

		// for an easier access by key
		var colors = { people_vaccinated: people_vaccinatedColor, people_fully_vaccinated: people_fully_vaccinatedColor };
		...
		var lastDate = new Date(covid_vaccine[covid_vaccine.length - 1].date);
		...
		for (var i = mapData.length - 1; i >= 0; i--) {
			if (mapData[i].people_vaccinated == 0) {
				mapData.splice(i, 1);
			}
		}


		var max = { people_vaccinated: 0, people_fully_vaccinated: 0 };
		//var maxPC = { people_vaccinated: 0, people_fully_vaccinated: 0 };

		// the last day will have most
		for (var i = 0; i < mapData.length; i++) {
			var di = mapData[i];
			if (di.people_vaccinated > max.people_vaccinated) {
				max.people_vaccinated = di.people_vaccinated;
			}
			if (di.people_fully_vaccinated > max.people_fully_vaccinated) {
				max.people_fully_vaccinated = di.people_fully_vaccinated;
			}
		}
		...
		container.tooltip.background.stroke = people_vaccinatedColor;
		...
		polygonSeries.dataFields.value = "people_vaccinatedPC";
		...
		bubbleSeries.dataFields.value = "people_vaccinated";
		...
		title.text = "COVID-19 U.S. Vaccination Data";
		title.align = "left";
		...
		absolutePerCapitaSwitch.rightLabel.text = "Population";
		...
		// buttons container (people_vaccinated/people_fully_vaccinated)
		...
		sizeSlider.startGrip.background.fill = people_vaccinatedColor;
		...
		sizeSlider.startGrip.background.states.getKey("hover").properties.fill = people_vaccinatedColor;
		sizeSlider.startGrip.background.states.getKey("down").properties.fill = people_vaccinatedColor;
		...
		filterSlider.startGrip.background.fill = people_vaccinatedColor;
		...
		filterSlider.startGrip.background.states.getKey("hover").properties.fill = people_vaccinatedColor;
		filterSlider.startGrip.background.states.getKey("down").properties.fill = people_vaccinatedColor;
		...
		lineChart.data = JSON.parse(JSON.stringify(covid_vaccine));
		...
		dateAxis.tooltip.background.fill = people_vaccinatedColor;
		dateAxis.tooltip.background.stroke = people_vaccinatedColor;
		...
		lineChart.cursor.lineX.stroke = people_vaccinatedColor;
		...
		// create series
		var people_vaccinatedSeries = addSeries("people_vaccinated", people_vaccinatedColor);
		people_vaccinatedSeries.tooltip.disabled = true;
		people_vaccinatedSeries.hidden = false;
		var people_fully_vaccinatedSeries = addSeries("people_fully_vaccinated", people_fully_vaccinatedColor);

		var series = { people_vaccinated: people_vaccinatedSeries, people_fully_vaccinated: people_fully_vaccinatedSeries };
		// add series
		function addSeries(name, color) {
			...
			series.tooltipText = "Total {name}: {valueY.formatNumber('#,###.00a')}";
		...
		function createColumnSeries() {
			columnSeries = {}
			columnSeries.people_vaccinated = addColumnSeries("people_vaccinated", people_vaccinatedColor);
			columnSeries.people_fully_vaccinated = addColumnSeries("people_fully_vaccinated", people_fully_vaccinatedColor);
		}
		...
		function addColumnSeries(name, color) {
			...
			series.tooltip.background.fill = am4core.color("#000000");
			series.tooltip.fontSize = "0.8em";
			series.tooltipText = "{name}: {valueY.previousChange.formatNumber('#,###.00a')}";
		...
		// BUTTONS
		// create buttons
		var people_vaccinatedButton = addButton("people_vaccinated", people_vaccinatedColor);
		var people_fully_vaccinatedButton = addButton("people_fully_vaccinated", people_fully_vaccinatedColor);

		var buttons = { people_vaccinated: people_vaccinatedButton, people_fully_vaccinated: people_fully_vaccinatedButton };
		...
		// change data type (people_vaccinated/people_fully_vaccinated)
		function changeDataType(name) {
			currentType = name;
			currentTypeName = name;
			if (name != "people_fully_vaccinated") {
				currentTypeName += "";
			}

			bubbleSeries.mapImages.template.tooltipText = "[bold]{name}: {value.formatNumber('#,###.00a')}[/] [font-size:10px]\n" + currentTypeName;
		...
		function setCountryData(countryIndex) {
			...
			for (var i = 0; i < lineChart.data.length; i++) {
				...
				if (countryData) {
					dataContext.people_vaccinated = countryData.people_vaccinated;
					dataContext.people_fully_vaccinated = countryData.people_fully_vaccinated;
					valueAxis.min = undefined;
					valueAxis.max = undefined;
				}
				else {
					dataContext.people_vaccinated = 0;
					dataContext.people_fully_vaccinated = 0;
					valueAxis.min = 0;
					valueAxis.max = 10;
				}
			}
		...
		function showWorld() {
			...
			for (var i = 0; i < lineChart.data.length; i++) {
				var di = covid_vaccine[i];
				var dataContext = lineChart.data[i];
				dataContext.people_vaccinated = di.people_vaccinated;
				dataContext.people_fully_vaccinated = di.people_fully_vaccinated;
				valueAxis.min = undefined;
				valueAxis.max = undefined;
			}
		...
		function updateTotals(index) {
			if (!isNaN(index)) {
				...
				for (var key in buttons) {
					...
					if (!isNaN(count)) {
						buttons[key].label.text = capitalizeFirstLetter(key) + ": " + numberFormatter.format(count, '#,###.00a');
					}
				}
				currentIndex = index;
			}
		...
		function updateMapData(data) {
			...
			bubbleSeries.dataItems.each(function(dataItem) {
				dataItem.dataContext.people_vaccinated = 0;
				dataItem.dataContext.people_fully_vaccinated = 0;
			})

			//maxPC = { people_vaccinated: 0, people_fully_vaccinated: 0 };
			...
			for (var i = 0; i < data.length; i++) {
				...
				if (image) {
					...
					image.dataItem.dataContext.people_vaccinated = di.people_vaccinated;
					image.dataItem.dataContext.people_fully_vaccinated = di.people_fully_vaccinated;
				}

				if (polygon) {
					polygon.dataItem.dataContext.people_vaccinatedPC = di.people_vaccinated / population * 100;
					polygon.dataItem.dataContext.people_fully_vaccinatedPC = di.people_fully_vaccinated / population * 100;

					/*if (polygon.dataItem.dataContext.people_vaccinatedPC > maxPC.people_vaccinated) {
                        maxPC.people_vaccinated = polygon.dataItem.dataContext.people_vaccinatedPC;
                    }
                    if (polygon.dataItem.dataContext.people_fully_vaccinatedPC > maxPC.people_fully_vaccinated) {
                        maxPC.people_fully_vaccinated = polygon.dataItem.dataContext.people_fully_vaccinatedPC;
                    }*/
				}
		...
		updateCountryName();
		changeDataType("people_vaccinated");
		...
		function populateCountries(list) {
			...
			for (var i = 0; i < list.length; i++) {
				var area = list[i]
				area['vaccinated']=format(area['people_vaccinated'])
				area['fully_vaccinated']=format(area['people_fully_vaccinated'])
				...
				$("<td>").addClass("value").appendTo(tr).html(area.vaccinated);
				$("<td>").addClass("value").appendTo(tr).html(area.fully_vaccinated);
			}
			$("#areas").DataTable({
				"paging": false,
				"select": true
